...
class Experiment:
    ...
    def __init__(self, file_base_name: str):
        ...
        self.embodiment_exercise = EmbodimentExerciseGrasp(self.config, enable_logging=True, log_name_base=file_base_name, is_eeg_version=False, serial_comm=self.serial_comm)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Non-EEG training session with grasp embodiment")
    ...
    if config.NUM_TOTAL_NORMAL_FINGER_TRIALS_PER_BLOCK != config.NUM_EACH_NORMAL_FINGER_PER_BLOCK * config.NUM_NORMAL_FINGERS:
        ...
        try:
            ...
            print("\n" + "="*50)
            print("NON-EEG TRAINING EXPERIMENT WITH GRASP EMBODIMENT ENDED")
            print("="*50)
            try:
                pygame.quit()
            except:
